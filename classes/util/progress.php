...
class progress {
    ...
    public function get_user_progress($courseid, $userid = null) {
        ...
        $record = $DB->get_record('myprogress_course_progress', ['courseid' => $courseid, 'userid' => $userid]);
    ...
    public function get_class_average($courseid) {
        ...
        $sql = 'SELECT AVG(progress) as average
                FROM {myprogress_course_progress}
    ...
    public function get_groups_average($courseid, $userid = null) {
        ...
        foreach ($usergroups[0] as $usergroup) {
            $sql = 'SELECT g.id, g.name, AVG(cp.progress) as average
                    ...
                    LEFT JOIN {myprogress_course_progress} cp ON cp.userid = gm.userid AND cp.courseid = :courseid
                    WHERE g.id = :groupid';
    ...
    public function get_cohorts_average($courseid, $userid = null) {
        ...
        foreach ($usercohorts as $usercohort) {
            $sql = 'SELECT c.id, c.name, AVG(cp.progress) as average
                    FROM {cohort} c
                    INNER JOIN {cohort_members} cm ON c.id = cm.cohortid
                    LEFT JOIN {myprogress_course_progress} cp ON cp.userid = cm.userid AND cp.courseid = :courseid
                    WHERE c.id = :cohortid';
